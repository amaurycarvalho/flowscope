...
Define as estratégias flexíveis de parsing de HTML tabular da B3 usadas na extração de documentos estruturados: busca por rótulo, parsing de tabelas com contexto, identificação de tipo de provento, limpeza de valores e fallbacks.
...
#### Scenario: Fallback por expressão regular
- **WHEN** o campo não é encontrado por rótulo nem por tabela, mas o HTML contém o padrão esperado
- **THEN** a estratégia DEVE retornar o valor capturado pelo fallback antes de retornar `None`
