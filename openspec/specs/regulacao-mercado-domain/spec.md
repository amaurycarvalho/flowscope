...
Define as entidades de domínio regulatório e de mercado da B3 — `CensuraPublica`, `CondicaoExcepcional`, `NoticiaB3`, `DocumentoMaterialFact` e suas subclasses — com os value objects `CodeCVM` e `CategoriaDocumento` e o método `to_text()` para indexação no VectorStore.
