...
- Preencher `Preço Típico` e `P / PT` a partir dos extremos (`Mín`/`Máx`) da janela de preços B3 já carregada em cache para a análise — restrita a 52 semanas antes da data de referência — quando o Fundamentus não fornecer `Min 52 sem`/`Max 52 sem`, sem novo acesso de mercado; se a janela em cache estiver vazia, manter `N/A`.
- **Cobertura de fallback (limites conhecidos)**: permanecem sem fallback B3/CVM e continuam `N/A` quando o Fundamentus não os fornece — `LPA`/`ROE`/`ROIC` (Papel) e `Qtd Imóveis`/`Cap Rate`/`Vacância Média` (FII). Para Papel, `P/L`, `Dividend Yield` e os dados de dividendo (última data-com, último dividendo, dividendo anterior) também dependem do Fundamentus, pois a série de proventos B3 (`type=41`) é FII-only. O `Preço Típico`/`P / PT` passa a ter fallback pela janela B3 em cache (que pode ser mais curta que 52 semanas). A matriz completa está em `design.md`.
...
- **Cache**: reutiliza as chaves existentes do informe B3 e do Informe Trimestral CVM; sem novas chaves obrigatórias. Toda origem consumida tem cache de arquivo bruto; permanecem sem memoização apenas os artefatos normalizados do Informe Trimestral e do Informe Anual da CVM (re-parseados do ZIP a cada chamada), já que `CvmAcionistasSource` cacheia o normalizado. A memoização de `obter_patrimonio` do repositório B3 é coberta pela task 2.2.
