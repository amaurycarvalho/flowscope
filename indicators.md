...
- **Documento vinculado:** quando o corpo da "Geral" é apenas uma URL em texto puro para o documento (ex.: "Esclarecimentos de questionamentos CVM/B3" → visualizador da CVM RAD), o conteúdo é baixado sob demanda (seleção e "Resumir pendentes") e anexado. O visualizador não serve o arquivo por GET: o PDF vem de um `POST` AJAX (`ExibirPDF`) em base64, resolvido com a sessão do visualizador e extraído com `pypdf`. O texto é persistido em `document-texts/NOTICIAS.json`; captcha, falha ou formato inesperado mantêm o corpo original.
